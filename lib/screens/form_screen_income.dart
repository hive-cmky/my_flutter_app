...
class IncomeCertificateForm extends StatefulWidget {
  const IncomeCertificateForm({super.key});

  @override
  State<IncomeCertificateForm> createState() => _IncomeCertificateFormState();
}

class _IncomeCertificateFormState extends State<IncomeCertificateForm> {
  ...
  // Income Details Controllers
  final List<TextEditingController> incomeControllers = List.generate(4, (_) => TextEditingController());
  ...
  void initState() {
    super.initState();
    _loadDistricts();
    // Add listeners to income controllers to calculate total income
    for (var controller in incomeControllers) {
  ...
  void _calculateTotal() {
    ...
    totalIncomeController.text = total.toStringAsFixed(2);
  }
  ...
  Future<void> _copyPresentToPermanent() async {
    ...
    if (presentTehsilCode != null) {
      ris = await DatabaseHelper.instance.getRIs(presentTehsilCode!);
    }
  ...
  Future<Map<String, dynamic>> _prepareFormData() async {
    ...
    final Map<String, dynamic> appForm = {
      ...
      if (anyPersonOtherThanApplicant ==
          ...
          .trim().toUpperCase(),
      "agricultureIncome": incomeControllers[0].text.trim(),
      "salaryIncome": incomeControllers[1].text.trim(),
      "TradeIncome": incomeControllers[2].text.trim(),
      "otherIncome": incomeControllers[3].text.trim(),
      "totalIncome": totalIncomeController.text.trim(),
  ...
  Future<void> _submitForm() async {
    ...
    final Map<String, String> displayData = {
      ...
      "Permanent Village": presentVillageNotInList
          ? presentVillageCustomController.text.toUpperCase()
          : (presentVillageName ?? ""),
      ...
      if (anyPersonOtherThanApplicant ==
          ...
          .trim()
          .toUpperCase(),
      "Agriculture Income": incomeControllers[0].text.trim(),
      "Salary/Wages": incomeControllers[1].text.trim(),
      "Trade/Business/Profession": incomeControllers[2].text.trim(),
      "Other Sources": incomeControllers[3].text.trim(),
      "Total Income": "₹ ${totalIncomeController.text.trim()}",
      "Purpose": purposeController.text.trim().toUpperCase(),
  ...
  Widget _buildIncomeSection() {
    final List<String> incomeLabels = [
      '1. Agriculture (including plantation, horticulture, dairying, poultry, fisheries etc.)',
      '2. Salary/Wages/Remuneration etc.',
      '3. Trade/Business/Profession (Please specify)',
      ...
      children: [
        ...
        ...List.generate(4, (index) {
          return Column(
            children: [
              TextFormField(
                controller: incomeControllers[index],
                decoration: InputDecoration(
                  labelText: incomeLabels[index],
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('Income Certificate'),
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            ...
            const Text(
              'APPLICATION FORM FOR ISSUE OF INCOME CERTIFICATE',
            ...
            if (_documentError != null) Text(_documentError!,
                style: const TextStyle(color: Colors.red, fontSize: 12)),
            ...
            const SizedBox(height: 16),
            Row(children: [Checkbox(value: agreedToDeclaration,
                  onChanged: (v) =>
